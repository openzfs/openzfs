...
zpool_vdev_clear(zpool_handle_t *zhp, uint64_t guid)
{
	...
	(void) snprintf(msg, sizeof (msg),
	    dgettext(TEXT_DOMAIN, "cannot clear errors for %"PRIu64""),
	    guid);
    ...
    char *pathname, size_t len)
{
	...
	if (dsobj == 0) {
		...
		(void) snprintf(pathname, len, "<metadata>:<0x%"PRIu64">", obj);
		return;
	}
	...
	if (ioctl(zhp->zpool_hdl->libzfs_fd,
		...
		(void) snprintf(pathname, len, "<0x%"PRIu64">:<0x%"PRIu64">",
		    dsobj, obj);
		return;
	}
	...
	if (ioctl(zhp->zpool_hdl->libzfs_fd, ZFS_IOC_OBJ_TO_PATH,
		...
		(void) snprintf(pathname, len, "%s:<0x%"PRIu64">", dsname, obj);
	}
	free(mntpnt);
}
