...
fzap_upgrade(zap_t *zap, dmu_tx_t *tx, zap_flags_t flags)
{
	...
	zap->zap_dbu.dbu_evict_func_async = zap_evict_async;
...
static void
zap_leaf_pageout_async(void *dbu)
{
	zap_leaf_t *l = dbu;
...
zap_create_leaf(zap_t *zap, dmu_tx_t *tx)
{
	...
	VERIFY(0 == dmu_buf_hold(zap->zap_objset, zap->zap_object,
	    ...
	    DMU_READ_NO_PREFETCH));
	dmu_buf_init_user(&l->l_dbu, NULL, zap_leaf_pageout_async, &l->l_dbuf);
...
zap_open_leaf(uint64_t blkid, dmu_buf_t *db)
{
	...
	if (winner != NULL) {
		...
		zap_leaf_pageout_async(&l->l_dbu);
		l = winner;
	}
